...
export default class Parser {
    ...
    private parseNewObjectExpr(): Stmt {
        ...
        const className =
            this.eatOnly(TokenType.Identifier, "Esperava o nome da classe")
                .value;
    ...
    private parseForStmt(): Stmt {
        ...
        const variable = {
            symbol:
                this.eatOnly(TokenType.Identifier, "Esperava uma váriavel")
                    .value,
        } as Identifier;
    ...
    private parseFuncDecl(): Stmt {
        ...
        const identifier =
            this.eatOnly(TokenType.Identifier, "Esperava um nome de função")
        ...
        if (this.at().type == TokenType.RParen) {
            ...
            if (this.at().type == TokenType.DoisPontos) {
                ...
                switch (tipo.type) {
                    ...
                    default:
                        throw reportError(
                            "Esperava um tipo válido de variável. Recebi: " +
                                returnType,
                            this.at().line,
                        );
                }
    ...
    private parseVarDecl(): Stmt {
        ...
        const identifier = this.eatOnly(
            TokenType.Identifier,
            "Esperava um nome de variável",
        )?.value;

        if (
            this.at().type == TokenType.EOL || this.at().type == TokenType.EOF
        ) {
            return {
                kind: "VarDecl",
                identifier,
            ...
            throw reportError(
                "Esperava ; ou = na declaração de váriavel",
                this.at().line,
            );
        }
    }
    ...
    private parseLogicalExpr(): Expr {
        ...
        while (
            this.at().value == "e" || this.at().value == "ou" ||
            this.at().value == "xou"
        ) {
            const operator = this.advance().value;
    ...
    private parsePrimaryExpr(): Expr {
        ...
        switch (tk) {
            ...
            default:
                throw reportError(
                    "Token inesperado: '" + this.at().value + "'",
                    this.at().line,
                );
        }
    }
